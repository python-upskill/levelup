...
class TestApplication(FalconTestCase):
    ...
    def test_arena(self, mock_requests):
        ...
        self.assertEqual(battle_result, arena.BattleResult(
            victory=arena.BattleResult.Victory(winner='orc2', rounds=1, ko=False),
            rounds=[arena.BattleResult.Round(round_number=1, attacker='orc', defender='orc2',
                                             damage=1, previous_hp=110, current_hp=109)]))
    ...
    def test_battle_results(self, mock_requests):
        ...
        battle_result_1 = to_battle_result(self.simulate_post(path="/arena",
                                                              json={
                                                                  'max_rounds': 1,
                                                                  'combatants': ["orc", "orc2"]
                                                              }).json)

        battle_result_2 = to_battle_result(self.simulate_post(path="/arena",
                                                              json={
                                                                  'max_rounds': 2,
                                                                  'combatants': ["orc2", "orc"]
                                                              }).json)

        battle_results_json = self.simulate_get(path="/battle_results", params={'limit': 2}).json
        battle_results = [to_battle_result(battle_result_json) for battle_result_json in battle_results_json]
