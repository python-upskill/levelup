...
class CombatantModel(BaseModel):
    ...
    def from_combatant(self, combatant: arena.Combatant) -> "CombatantModel":
        return self.create(
...
class BattleResultModel(BaseModel):
    ...
    def from_battle_result(
        self, battle_result: arena.BattleResult, combatant_model_winner: CombatantModel
    ) -> "BattleResultModel":
        return self.create(
    ...
    def to_battle_result(
        self, battle_result_round_models: "BattleResultRoundModels"
    ) -> arena.BattleResult:
        return arena.BattleResult(
            victory=arena.BattleResult.Victory(
                rounds=self.rounds, winner=self.winner.name, ko=self.ko
            ),
            rounds=[
                battle_result_round_model.to_battle_result_round()
...
class BattleResultRoundModel(BaseModel):
    ...
    class Meta:
        indexes = ((("battle_id", "round_number"), True),)

    def from_battle_result_round(
        self,
        battle_result_round: arena.BattleResult.Round,
        battle_result_model: BattleResultModel,
        combatant_model_attacker: CombatantModel,
        combatant_model_defender: CombatantModel,
    ) -> "BattleResultRoundModel":
        return self.create(
...
def read_combatant(combatant_name: str) -> arena.Combatant:
    combatant_model: CombatantModel = CombatantModel.get_or_none(
        CombatantModel.name == combatant_name
    )
...
def write_battle_result(battle_result: arena.BattleResult) -> None:
    combatant_models: dict = {
        battle_result.rounds[0].attacker: CombatantModel.get(
            CombatantModel.name == battle_result.rounds[0].attacker
        ),
        battle_result.rounds[0].defender: CombatantModel.get(
            CombatantModel.name == battle_result.rounds[0].defender
        ),
    }
    battle_result_model = BattleResultModel().from_battle_result(
        battle_result, combatant_models.get(battle_result.victory.winner)
    )
    for battle_result_round in battle_result.rounds:
        BattleResultRoundModel().from_battle_result_round(
            battle_result_round,
            battle_result_model,
...
def __to_battle_result(battle_result_model: BattleResultModel) -> arena.BattleResult:
    return battle_result_model.to_battle_result(
        BattleResultRoundModel.select().where(
            BattleResultRoundModel.battle == battle_result_model
        )
    )


def read_battle_result_latest(limit: int) -> arena.BattleResults:
    battle_result_models: BattleResultRoundModels = BattleResultModel.select().order_by(
        BattleResultModel.created_date.desc()
    ).limit(limit)
    return [
        __to_battle_result(battle_result_model)
        for battle_result_model in battle_result_models
    ]
